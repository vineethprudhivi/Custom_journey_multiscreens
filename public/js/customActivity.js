...
exports.webhookSubmit = async function (req, res) {
    try {
        ...
        // Track save outcomes to return to the UI
        ...
        if (hasCredentials) {
            ...
            // Format date as SFMC-compatible string (works for both Date and Text fields)
            const now = new Date();
            ...
            try {
                console.log(`Attempting Job Tracking upsert to DE '${jobTrackingDEKey}' with jobId=${jobId}`);
                ...
                const errMsg = err.response ? JSON.stringify(err.response.data) : err.message;
                console.error(`Failed to save to Job Tracking DE '${jobTrackingDEKey}':`, errMsg);
